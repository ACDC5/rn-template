...
import Network, { NetInfoSubscription } from '@react-native-community/netinfo';
...
export function useSWRNativeRevalidate<Data = any, Error = any>(props: Props<Data, Error>) {
  ...
  useEffect(() => {
    let unsubscribeReconnect: NetInfoSubscription;
    ...
    const onFocus = () => {
      ...
      const isThrottled =
        focusThrottleInterval && lastFocusedAt.current && Date.now() - lastFocusedAt.current <= focusThrottleInterval;
  ...
  }, [focusThrottleInterval, revalidateOnFocus, revalidateOnReconnect]);
}
